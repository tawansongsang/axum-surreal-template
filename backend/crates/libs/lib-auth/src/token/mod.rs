...
use hmac::{Hmac, Mac};
use lib_utils::{b64::{b64u_decode_to_string, b64u_encode}, time::{now_utc_plus_sec_str, parse_utc, now_utc}};
use sha2::Sha512;
use uuid::Uuid;
...
fn inner_generate_token(
    ...
    let exp = now_utc_plus_sec_str(duration_sec).map_err(|ex| Error::InvalidDuration(ex.to_string()))?;
...
fn inner_validate_token_sign_and_exp(
    origin_token: &Token, 
    salt: Uuid, 
    key: &[u8]
) -> Result<()> {
    // -- Validate signature.
    let new_sign_b64u = inner_token_sign_into_b64u(&origin_token.ident, &origin_token.exp, salt, key)?;
...
mod tests {
	...
	fn test_validate_web_token_err_expired() -> Result<()> {
		...
		let token_key = &auth_config().TOKEN_KEY;
		let fx_token =
			inner_generate_token(fx_user, fx_duration_sec, fx_salt, token_key)?;
